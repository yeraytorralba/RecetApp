...
public class MedidasService implements IMedidasService {
		...
		try {
			if (medida == null || medida.getNombre() == null) {
				throw new ServiceException(
						"Error al crear la medida: La medida no puede ser nula.");
			}
	...
	public void modificarMedida(MedidaItem medida) throws ServiceException {
		try {
			...
			PreparedStatement statement = conn
					.prepareStatement("UPDATE medidas SET nombre = ?, abreviatura = ? WHERE id = ?");
			statement.setString(1, medida.getNombre());
			statement.setString(2, medida.getAbreviatura());
			...
			throw new ServiceException(
					"Error al modificar la medida: La medida no puede ser nula.");
		}
	}
	...
	public void eliminarMedida(Long id) throws ServiceException {
		try {
			Connection conn = BaseDatos.getConnection();
			PreparedStatement statement = conn
					.prepareStatement("DELETE FROM medidas WHERE id = ?");
			statement.setLong(1, id);
			statement.executeUpdate();
			statement.close();
		} catch (SQLException e) {
			throw new ServiceException("Error al eliminar la medida '" + id
					+ "': " + e.getMessage());
		} catch (NullPointerException e) {
			throw new ServiceException(
					"Error al eliminar la medida: La medida no puede ser nula.");
		}
	}
	...
	public List<MedidaItem> listarMedidas() throws ServiceException {
		...
		try {
			Connection conn = BaseDatos.getConnection();
			PreparedStatement statement = conn
					.prepareStatement("SELECT * FROM medidas ORDER BY nombre");
	...
	public MedidaItem obtenerMedida(Long id) throws ServiceException {
		if (id == null) {
			throw new ServiceException(
					"Error al recuperar la medida: Debe especificar un id válido.");
		}
		MedidaItem medida = null;
		try {
			Connection conn = BaseDatos.getConnection();
			PreparedStatement statement = conn
					.prepareStatement("SELECT * FROM medidas WHERE id = ?");
			statement.setLong(1, id);
			ResultSet rs = statement.executeQuery();
			if (rs.next()) {
			...
			throw new ServiceException("Error al recuperar la medida con ID '"
					+ id + "'", e);
		}
		return medida;
	}
