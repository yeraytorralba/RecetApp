...
public class CategoriasService implements ICategoriasService {
		...
		try {
			if (categoria == null || categoria.getDescripcion() == null) {
				throw new ServiceException(
						"Error al crear la categoría: La categoría no puede ser nula.");
			}
	...
	public void modificarCategoria(CategoriaItem categoria)
		...
		try {
			...
			throw new ServiceException(
					"Error al modificar la categoría: La categoría no puede ser nula.");
		}
	}
	...
	public void eliminarCategoria(Long id) throws ServiceException {
		try {
			...
			throw new ServiceException("Error al eliminar la categoría '" + id
					+ "': " + e.getMessage());
		} catch (NullPointerException e) {
			throw new ServiceException(
					"Error al eliminar la categoría: La categoría no puede ser nula.");
		}
	}
	...
	public List<CategoriaItem> listarCategorias() throws ServiceException {
		...
		try {
			Connection conn = BaseDatos.getConnection();
			PreparedStatement statement = conn
					.prepareStatement("SELECT * FROM categorias ORDER BY descripcion");
	...
	public CategoriaItem obtenerCategoria(Long id) throws ServiceException {
		if (id == null) {
			throw new ServiceException(
					"Error al recuperar la categoría: Debe especificar un id válido.");
		}
		...
		try {
			...
			throw new ServiceException(
					"Error al recuperar la categoria con ID '" + id + "'", e);
		}
		return categoria;
	}
